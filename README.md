...
13 个分阶段角色、强制证据引用、多空/风险辩论和角色独立记忆采用内置推荐配置，不在设置页暴露。系统为行情、技术指标、宏观、筛选、账户、优化器和上游 Agent 报告生成稳定证据 ID；每条事实判断及每个组合决策必须引用有效 ID，伪造或漏引会重试后阻断。记忆按“市场 + 角色”隔离保存在 `runtime/trading/agent_memory/`，每个角色只能读取自己的有限历史，并且记忆不能冒充本轮证据。
