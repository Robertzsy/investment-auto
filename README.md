...
14 个分阶段角色、强制证据引用、多空/风险顺序辩论采用内置推荐配置，不在设置页暴露。系统为行情、技术指标、逐标的新闻、基本面、宏观、筛选、账户和上游 Agent 报告生成稳定证据 ID；每条事实判断及每个组合决策必须引用有效 ID，伪造、漏引或跳过直接上游会重试后阻断。Agent 链失败时所有主观交易关闭，但硬止损、移动止损和最大回撤清仓仍可由代码独立执行。
