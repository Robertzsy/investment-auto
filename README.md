...
Windows Release 包提供 `InvestmentAuto.exe`。首次解压后运行一次 `Setup-Windows.cmd`，配置好 `.env`，以后直接双击 `InvestmentAuto.exe` 即可统一启动投资 Agent、调度器、AI 对话和仪表盘。启动器会识别已经运行的服务，不会重复创建进程。

启动器内可以启动/停止服务、打开管理页面，并可启用“登录 Windows 后自动启动”。开机自启写入当前用户启动项，不需要管理员权限；API Key、持仓、报告和日志仍保存在项目目录，不会写入 EXE。

如果使用现有本地部署，也可以只把 `InvestmentAuto.exe` 放进项目根目录。启动器依次查找 EXE 所在目录、`INVESTMENT_AUTO_HOME` 和 `D:\investment-auto`。
...
- 对话管理 Agent 可以读取、新建和修改项目目录内任意文本文件，包括代码、配置、提示词、运行时数据、密钥、账户与管理模块。
- 代码修改要求读取后的 SHA-256，保存版本化备份并执行全量测试；测试失败自动回滚。
- 管理 Agent 仍没有任意 Shell；文件工具不能越出项目根目录。
